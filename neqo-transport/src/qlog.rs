...
use crate::connection::Path;
use crate::frame::{self, decode_frame, Frame};
...
pub fn packet_sent(qlog: &Option<NeqoQlogRef>, now: Instant, hdr: &PacketHdr, body: &[u8]) {
    if let Some(qlog) = qlog {
        ...
        let packet_type = pkt_type_to_qlog_pkt_type(&hdr.tipe);
        ...
        qlog.trace.push_transport_event(
            ...
            EventData::PacketSent {
                packet_type,
                header: PacketHeader {
                    packet_number: hdr.pn.to_string(),
                    packet_size: None,
                    payload_length: None,
                    version: None,
                    scil: None,
...
pub fn packet_received(qlog: &Option<NeqoQlogRef>, now: Instant, hdr: &PacketHdr, body: &[u8]) {
    if let Some(qlog) = qlog {
        let mut qlog = qlog.borrow_mut();
        let elapsed = now.duration_since(qlog.zero_time);
...
fn pkt_type_to_qlog_pkt_type(ptype: &PacketType) -> qlog::PacketType {
    match ptype {
        PacketType::Initial(_) => qlog::PacketType::Initial,
